...
test('findTriggeringMembers creates one lifecycle cycle for three unlimited cancellations in seven days', () => {
    const detections = findTriggeringMembers([
        cancellation(100, '2026-05-01T00:00:00.000Z'),
        cancellation(100, '2026-05-03T00:00:00.000Z'),
        cancellation(100, '2026-05-07T00:00:00.000Z')
    ...
    assert.equal(detections[0].latestLateCancellationAt, '07-05-2026 05:30:00');
...
test('findTriggeringMembers ignores non-unlimited memberships and existing cycles', () => {
    ...
    const detections = findTriggeringMembers([
        ...
        cancellation(100, '2026-05-03T00:00:00.000Z'),
        cancellation(100, '2026-05-07T00:00:00.000Z'),
        cancellation(200, '2026-05-01T00:00:00.000Z', 'Studio 10 Pack'),
        cancellation(200, '2026-05-03T00:00:00.000Z', 'Studio 10 Pack'),
        cancellation(200, '2026-05-07T00:00:00.000Z', 'Studio 10 Pack')
    ], existing, new Date('2026-05-08T00:00:00.000Z'));
